...
class BaseAnseriniIndex(BaseIndex):
    ...
    """
    def __init__(self, collection_dir:str, index_dir:str) -> None:
        """
        Args:
    ...
    def convert_retrieval_result(self, tmp_retrieval_result_path:str, verifier:Optional[BasePostVerifier]=None) -> RETRIEVAL_MAPPING:
        """
    ...
    def prepare_query(self, query_path, tmp_query_dir, query_token_ids=None, query_token_weights=None):
        """
        ...
        if len(qcontents) > 10000:
            ...
            for qidx, query in enumerate(qcontents):
                if qidx % 5000 == 0:
                    if qidx > 0:
                        g.close()
                    g = open(os.path.join(query_split_dir, f"{str(qidx // 5000)}.tsv"), "w")
                g.write("\t".join([str(qidx), query]) + "\n")
            g.close()
            return True, query_split_dir

        else:
            os.makedirs(tmp_query_dir, exist_ok=True)
            query_path = os.path.join(tmp_query_dir, "queries.tsv")
            with open(query_path, "w") as f:
                for qidx, qcontent in enumerate(qcontents):
                    f.write("\t".join([str(qidx), qcontent]) + "\n")
...
class AnseriniImpactIndex(BaseAnseriniIndex):
    ...
    def __init__(self, collection_dir:str, index_dir:str) -> None:
        ...
        Args:
            ...
            index_dir: the folder to store the lucene index file; usually :py:obj:`models.BaseModel.BaseModel.index_dir`
        """
        super().__init__(collection_dir, index_dir)


    def _quantize(self, token_weights):
        if hasattr(self, "quantize_bit") and self.quantize_bit > 0:
            scale = (1 << self.quantize_bit) / token_weights.max()
        else:
            scale = 100
    ...
    def fit(self, text_token_ids:np.ndarray, text_token_weights:np.ndarray, quantize_bit:int, tokenizer:Any, granularity:str, subword_to_word:Callable, stop_words:set, reduce:str, thread_num:int=32, enable_build_collection:bool=True, enable_build_index:bool=True, language:str="eng", **kwargs):
        """
        ...
        if subword_to_word is not None:
            assert granularity == "word", "The subword_to_word can only be callable when using the word granularity!"
        ...
        if enable_build_collection:
            ...
            text_token_weights = self._quantize(text_token_weights)
    ...
    def _search(self, query_path, output, hits, language):
        # subprocess.run(f"""
    ...
    def search(self, query_token_ids, retrieval_result_path:str, hits:int, query_token_weights:Optional[np.ndarray]=None, query_path:Optional[str]=None, tmp_query_dir:Optional[str]=None, language:str="eng", verifier:Optional[BasePostVerifier]=None, **kwargs) -> RETRIEVAL_MAPPING:
        """
        Search by Anserini.
        ...
        split, query_path_or_dir = self.prepare_query(query_path=query_path, query_token_ids=query_token_ids, query_token_weights=query_token_weights, tmp_query_dir=tmp_query_dir)
        ...
        if split:
            ...
            for query_path in tqdm(os.listdir(query_path_or_dir), ncols=100, desc="Searching"):
                ...
                res = self.convert_retrieval_result(tmp_retrieval_result_path, verifier=verifier)
                retrieval_result.update(res)

        else:
            self._search(query_path_or_dir, tmp_retrieval_result_path, hits, language)
            retrieval_result = self.convert_retrieval_result(tmp_retrieval_result_path, verifier=verifier)
...
class AnseriniBM25Index(BaseAnseriniIndex):
    ...
    Anserini BM25 index.
    """
    def __init__(self, collection_dir, index_dir) -> None:
        """
        Args:
            collection_dir: the json collections folder generated by :func:`utils.index.AnseriniImpactIndex.fit` and :func:`utils.index.AnseriniBM25Index.fit`
            index_dir: the folder to store the lucene index file; usually :py:obj:`models.BaseModel.BaseModel.index_dir`
    ...
    def fit(self, text_path:str=None, text_cols:list=None, text_token_ids:Optional[np.ndarray]=None, tokenizer:Any=None, thread_num:int=32, enable_build_collection:bool=True, enable_build_index:bool=True, language:str="eng", stop_words:Optional[set]=None, granularity:Optional[str]=None, **kwargs):
        """
        ...
        if enable_build_collection:
            ...
            # NOTE: empty the directory if there are some files
            if not isempty(self.collection_dir):
                shutil.rmtree(self.collection_dir)
                os.makedirs(self.collection_dir)

            if text_token_ids is None:
                # in case the text is not pretokenized
                build_default_collection(text_path=text_path, output_dir=self.collection_dir, text_cols=text_cols)
            else:
    ...
    def _search(self, query_path, output, k1, b, hits, language, pretokenized=False):
        if pretokenized:
            # subprocess.run(f"""
            #     python -m pyserini.search.lucene \
            #     --index {self.index_dir} \
    ...
    def search(self, query_path:str, retrieval_result_path:str, hits:int, query_token_ids:Optional[np.ndarray]=None, tmp_query_dir:Optional[str]=None, language:str="eng", k1:float=0.82, b:float=0.68, verifier:Optional[BasePostVerifier]=None, **kwargs) -> RETRIEVAL_MAPPING:
        """
        Search by Anserini.
        ...
        """
        tmp_retrieval_result_path = f"{retrieval_result_path}.tmp"
        split, query_path_or_dir = self.prepare_query(query_path=query_path, query_token_ids=query_token_ids, query_token_weights=None, tmp_query_dir=tmp_query_dir)
...
class BeamDecoder():
    ...
    def update_beams(self, beam_tokens, beam_scores, beam_indices, input_ids, model, model_kwargs, past_key_values, max_new_tokens, constrain_index:Union[TrieIndex, WordSetIndex]):
        ...
        for batch_id, (batch_beam_tokens, batch_beam_scores, batch_beam_indices) in enumerate(zip(beam_tokens, beam_scores, beam_indices)):
            ...
            if self.constrain_index_type == "wordset":
                prev_words_this_batch_new = [None for _ in range(self.num_beams)]
                prev_text_indices_this_batch_new = [None for _ in range(self.num_beams)]
                prefixes_this_batch_new = [None for _ in range(self.num_beams)]

                # by default we just slice out the beam_index result
                prev_words_this_batch = self.prev_words[global_batch_idx]
                prev_text_indices_this_batch = self.prev_text_indices[global_batch_idx]
                prefixes_this_batch = self.prefixes[global_batch_idx]
            ...
            for beam_rank, (beam_token, beam_score, beam_index) in enumerate(zip(batch_beam_tokens, batch_beam_scores, batch_beam_indices)):
                ...
                if self.constrain_index_type == "wordset":
                    ...
                    # t13 = time.time()
                    if (prev_text_indices is not None and len(prev_text_indices) == 1 and self.do_early_stop and self.cur_len >= self.early_stop_start_len) or beam_token == constrain_index.eos_token_id:
                        # when there is only one document containing the generated words, add to beam
                        # when there are duplicated docids, eos will be decoded at the end, add to beam too
                ...
                # when cur_new_tokens has reached the limits, add to beam regardless of eos token
                    ...
                    if self.do_early_stop and self.cur_len >= self.early_stop_start_len:
                        # add to beam when there is only one document corresponding to the generated tokens
                        input_id = input_ids[batch_beam_idx].tolist() + [beam_token]
        ...
        while self.num_left_batch and self.cur_new_tokens < max_new_tokens:
            ...
            if self.constrain_index_type == "trie":
                ...
                for batch_id in range(self.batch_size):
                    ...
                    prefixes = self.prefixes[global_batch_idx]
                    for beam_id in range(self.prev_num_beams):
                        ...
                        if beam_score < -1e9:
                            ...
                            if text_indices is not None:
                                valid_tokens = constrain_index.get_valid_tokens_from_doc(text_indices[global_batch_idx], prev_words[beam_id], prefixes[beam_id])
                            else:
...
def build_default_collection(text_path, output_dir, text_cols=[-1], max_docs_per_file=1000000):
    file_index = 0
    with open(text_path, encoding='utf-8') as f:
        for i, line in enumerate(tqdm(f, ncols=100, desc="Generating JSONs")):
            columns = line.split('\t')
            ...
            if i % max_docs_per_file == 0:
                ...
                file_index += 1
            output_dict = {'id': i, 'contents': doc_text}
            output_jsonl_file.write(json.dumps(output_dict) + '\n')
    output_jsonl_file.close()


def build_impact_collection(token_ids, token_weights, text_start_idx, output_json_path, tokenizer, subword_to_word, stop_words, reduce):
    """
...
def build_pretokenized_collection(token_ids, text_start_idx, output_json_path, tokenizer, stop_words, granularity):
    """
    generate jsonl files in multiple threads
    ...
    with open(output_json_path, "w") as g:
        for idx, token_id in enumerate(tqdm(token_ids, desc="Generating JSONs", ncols=100, leave=False)):
            if granularity == "word":
                output = tokenizer.decode(token_id, skip_special_tokens=True).split(" ")
            else:
                output = tokenizer.convert_ids_to_tokens(token_id, skip_special_tokens=True)

            if len(stop_words):
                tokens = [x for x in output if x not in stop_words]
