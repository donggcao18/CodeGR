...
testset = trainset.map(lambda example: {"query": process_query(example[args.query_column]), "doc": process_doc(example[args.doc_column])})
...
with jsonlines.open(os.path.join(args.save_dir, f"{language}_train.json"), mode='w') as writer:
    writer.write_all(train_data)
    
with jsonlines.open(os.path.join(args.save_dir, f"{language}_test.json"), mode='w') as writer:
